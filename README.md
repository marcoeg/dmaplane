...
dmaplane exposes a character device (`/dev/dmaplane`) controlled via ioctl, implementing the buffer orchestration layer that sits beneath transport libraries and includes: DMA allocation, NUMA-aware placement, RDMA memory registration, dma-buf zero-copy sharing, GPU BAR pinning, credit-based flow control, and disaggregated KVCache transfer.


**Read the Paper:** [*The DMA Streaming Framework: Kernel-Level Buffer Orchestration for High-Performance AI Data Paths*](arxiv_paper.pdf); submitted to arXiv.
